...
def plot_cost_max_iter(y,x,regression_type,lambda_ = 0):
    ...
    LEGEND_TAG = [['gamma_ ' + str(i) +' = '+str(gammas[i]) + ' //  ' + str(all_max_iterations_with_epsilon_precision[i])] for i in range(len(gammas))] 
...
all_weights, all_losses, gammas, all_max_iterations_with_epsilon_precision = plot_cost_max_iter(y,x,"reg_logistic_regression", 0)